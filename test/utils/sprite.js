/**
 * Deterministic text rendering for the pixel fixtures.
 *
 * Ported from chartjs-test-utils v0.5.0 (MIT). Real font rasterization differs
 * between browsers and platforms, so fixtures that draw text patch the context
 * to blit characters from a bitmap sprite sheet instead.
 */
...
function getChar(asciiCode) {
  ...
  return { h: fontHeight, sx: x * cellWidth, sy: y * cellHeight, w: fontWidth[index] }
}
...
function baselineOffset(baseline) {
  switch (baseline) {
    case 'top':
    case 'hanging':
      return fontHeight
...
function spriteWrite(text, x, y) {
  ...
  y -= baselineOffset(this.textBaseline)
